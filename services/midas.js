...
class MidasScraper {
  ...
  async login(page) {
    ...
    this.step = 'start opening modas login page'
  ...
  async buyPubg(playerId, productAmount, cb) {
    ...
    if (!availableAmount.includes(productAmount)) {
      return cb({ status: 'error', step: this.step, error: 'product not found' })
    }
    ...
    try {
      ...
      page.on('response', async (response) => {
        if (response.url().includes('getCharac')) {
          ...
          if (jsonres.ret == 2002) {
            return cb({ status: 'error', step: this.step, error: 'playerid not found' })
          }
        }
      });
      ...
      return cb({ status: 'error', step: this.step, error: 'catch: ' + error })
    }
    finally {
  ...
  async loginAndCheckoutUnipin() {
    ...
    this.step = 'login unipin success'
